...
function WorkerModule:LoadModule(ModuleScript : ModuleScript)
	if not ModuleScript:IsA("ModuleScript") then error("AddTask did not receive a module script") end

	local Clone : ModuleScript = ModuleScript:Clone()
	local Function = require(Clone)
	if type(Function) ~= "function" then error("Module script did not return a function") end
	task.delay(0,function() Clone:Destroy() end)
	...
	local function GetCurrentNumber()
		return Number
	end
	...
	local function SetNumber(NewNumber)
		Number = NewNumber
	end
	...
	local function CreateWorker(ModuleScript : ModuleScript)
		local Worker = script.WorkerTemplate.Actor:Clone()

		local ModuleScript = ModuleScript:Clone()
		ModuleScript.Parent = Worker
	...
	function ModuleTable:SetMaxWorkers(_MaxWorkers)
		if type(_MaxWorkers) ~= "number" then error("Unable to asign MaxWorkers. Number expected, got "..type(_MaxWorkers)) end
		if _MaxWorkers ~= math.clamp(math.round(_MaxWorkers),1,math.huge) then error("Unable to asign MaxWorkers. Value must be an integer between 1 and inf") end
		...
		task.spawn(function()
			...
			for i, v in ipairs(WorkerFolder:GetChildren()) do 
				if not v:IsA("Actor") then continue end
				local WorkerId = tonumber(v.Name)
				if WorkerId >_MaxWorkers then DeleteWorker(WorkerId) end
			end
		end)
	end
	...
	function ModuleTable:Work()
		...
		assert(not IsWorking,"ModuleTable:Work() was called before the previous tasks were completed. This can cause errors or wrong results. If this was caused because of an error in the ModuleScript, you can ignore it")
		IsWorking = true
